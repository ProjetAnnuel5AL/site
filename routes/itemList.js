  ...
  app.get('/itemList', function(req, res, next) {
    ...
    var limit = (req.params.page -1)*20
      ...
      rp({
        ...
        if (body.code == 0) {
          //console.log(body)
          for (var item in body.result.list) {
            if(prixMax < body.result.list[item].priceItem){
              prixMax = body.result.list[item].priceItem;
            }
            if(prixMin > body.result.list[item].priceItem){
              prixMin = body.result.list[item].priceItem;
            }
            if(!cities.includes(body.result.list[item].cityItem)){
              cities.push(body.result.list[item].cityItem);
            }
            if(producers.filter(producers => (producers.id === body.result.list[item].idProducer)).length===0){
              producers.push({id:body.result.list[item].idProducer, name:body.result.list[item].producerFirstName+" "+body.result.list[item].producerName});
            }
            if(categories.filter(categories => (categories.id === body.result.list[item].idCategory)).length===0){
              categories.push({id:body.result.list[item].idCategory, name:body.result.list[item].nameCategory});
            }
            if(products.filter(products => (products.id === body.result.list[item].idProduct)).length===0){
              products.push({id:body.result.list[item].idProduct, name:body.result.list[item].nameProduct, idCat:body.result.list[item].idCategory, nameCat:body.result.list[item].nameCategory});
            }

          }
         
          res.render('itemList.ejs', { msgError: "", urlApi: urlApi, categories: categories, products: products, producers: producers, cities: cities, listTab: body.result.list, prixMin: prixMin, prixMax: prixMax, session : req.session, currentPage: req.params.page, nbItems : body.result.nbTotalItem });
        } else {
